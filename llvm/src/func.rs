...
use crate::{compiler_error, stdlib_defs::get_stdlib_function};
use lexer::types::Datatype;
use parser::nodes::{
    ExpressionParserNode, FunctionCallParserNode, FunctionParserNode, ReturnNode, ValueParserNode,
};
...
impl<'ctx> CodeGen<'ctx> {
    ...
    pub fn def_extern(&self, func_name: &str, imported: &Vec<String>) -> FunctionValue<'ctx> {
        ...
        if import_path.is_none() {
            compiler_error(&format!("Module '{}' not imported", imported.join(":")));
        }
        ...
        let func_def = if import_path.first().unwrap() == "std" {
            if let Some(func) = get_stdlib_function(&internal_func_name) {
                ...
                compiler_error(&format!("Function '{}' not found in stdlib", func_name));
            }
        } else {
    ...
    pub fn add_func_call(
        ...
        let function = if let Some(imported) = &func_node.imported {
            ...
            compiler_error(&format!("Function {} not found", func_node.func_name));
        };
